...
import { FileText, Download, Eye, Briefcase, Building2 } from 'lucide-react';
...
export default function ClientDetailPage() {
  ...
  // Хелпер для названия типа
  const getDocLabel = (type) => DOC_TYPES.find(t => t.value === type)?.label || type;
  ...
  return (
    <Layout>
                ...
                <div>
                    ...
                    <div className="flex justify-between items-center mb-4 bg-gray-100 p-2 rounded-lg">
                        <div className="flex items-center gap-2 px-2">
                            <button className="w-8 h-8 bg-white rounded border border-gray-300 flex items-center justify-center text-gray-500 shadow-sm">+</button>
                            <input placeholder="поиск" className="bg-transparent outline-none text-sm min-w-[200px]" />
                        </div>
                        <div className="text-xs text-gray-500 mr-4">Всего документов: {documents.length}</div>
                    </div>
      ...
      {/* Модалки */}
      {isUploadOpen && <UploadDocModal onClose={() => setIsUploadOpen(false)} onUpload={handleUpload} />}
