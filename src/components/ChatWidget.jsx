...
export default function ChatWidget({ applicationId }) {
    ...
    useEffect(() => {
        load();
        // Простейший поллинг (обновление раз в 5 сек) для MVP
    ...
    const send = async () => {
        ...
        try {
            await api.post('/messages', { 
                // Приводим к числу, так как DTO ожидает int, а из URL может прийти строка "5"
                application_id: parseInt(applicationId), 
                body: text 
                // document_id не отправляем, если нет файла
            });
            setText('');
            load();
        } catch (e) {
    ...
    return (
        <>
            <div className="flex-1 p-4 overflow-y-auto bg-white space-y-3">
                ...
                {messages.map(m => (
                    <div key={m.id} className={`p-3 rounded-lg max-w-[85%] text-sm ${
                        m.sender_user?.role === 'admin' 
