...
import { callBaseApi, Lead } from '../api/callBaseApi';
import { useDebounce } from '@shared/hooks/useDebounce';

export const CallBasePage = () => {
    ...
    const { data: leads = [], isLoading } = useQuery({
        queryKey: ['callbase', 'leads'],
        queryFn: callBaseApi.getLeads,
    });

    const statusMutation = useMutation({
        mutationFn: ({ id, status }: { id: number; status: string }) =>
            callBaseApi.updateStatus(id, status),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['callbase'] }),
    });

    const commentMutation = useMutation({
        mutationFn: ({ id, comment }: { id: number; comment: string }) =>
            callBaseApi.updateComment(id, comment),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['callbase'] }),
    });
    ...
    const filteredLeads = leads.filter(lead =>
        ...
        lead.inn.includes(debouncedSearch) ||
        ...
        <div>
            ...
            {showScript && (
                <div className="bg-blue-50 border border-blue-100 p-6 rounded-xl mb-6 animate-fade-in">
                    ...
                    <p className="text-blue-900 text-sm mb-2">
                        "Добрый день, [Имя]! Меня зовут [Ваше Имя], компания Лидер-Гарант. Мы финансовый
                        маркетплейс, аккредитованный партнер 50 банков."
                    </p>
                    <p className="text-blue-900 text-sm">
                        "Вижу, вы выиграли тендер [Номер]. Скажите, вопрос с банковской гарантией уже решили или
                        актуально предложение с тарифом от 1.5%?"
                    </p>
                </div>
            )}
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 flex gap-4 items-center">
                <div className="relative flex-1">
                    ...
                    <input
                        placeholder="Поиск по ИНН, названию или телефону..."
                                ...
                                <tr key={lead.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-6 py-4">
                                        ...
                                        <div className="text-xs text-gray-400 flex items-center gap-1 mt-1">
                                            ...
                                            <Copy
                                                size={12}
                                                className="cursor-pointer hover:text-[#3CE8D1]"
                                                onClick={() => alert('ИНН скопирован')}
                                            />
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <a
                                                href={`tel:${lead.phone}`}
                                                className="font-medium text-gray-800 hover:text-[#3CE8D1] whitespace-nowrap"
                                            >
                                                {lead.phone}
                                            </a>
                                    ...
                                    <td className="px-6 py-4">
                                        <select
                                            ...
                                            className={`px-3 py-1.5 rounded-lg text-xs font-bold border-none focus:ring-2 focus:ring-[#3CE8D1] cursor-pointer ${statusConfig[lead.status].bg
                                    ...
                                    <td className="px-6 py-4">
                                        <input
                                            ...
                                            onBlur={(e) => commentMutation.mutate({ id: lead.id, comment: e.target.value })}
                                            placeholder="Заметка..."
